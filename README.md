# ILIAS Deadline Checker
 ### **"Inoffizielles Tool von Studenten für Studenten – kein offizielles Angebot der FH Aachen."**

Diese Erweiterung zeigt dir alle anstehenden Praktikums-Fristen der FH Aachen (oder anderen Hochschulen) mit einem Klick.

########################################################################################################################################################################

### 1. Installation
✅ **Getestet und funktioniert dauerhaft in:** Chrome, Edge, Opera, Vivaldi und Brave.
⚠️ **Firefox:** Funktioniert technisch auch, wird aber hier **nicht offiziell unterstützt** Grund: (Erweiterung muss nach jedem Browser-Neustart neu geladen werden).

**Für Chrome / Edge / Opera / Vivaldi / Brave:**
1. Öffne im Browser `chrome://extensions/` (oder `edge://extensions/`).
2. Aktiviere oben rechts den **Entwicklermodus**.
3. Klicke auf **Entpackte Erweiterung laden** und wähle den **"ilias_checker" Ordner** aus.
4. Pinne das Icon neben deine Adressleiste.

**Für Firefox (Eingeschränkt/nicht empfohlen):**
1. Öffne im Browser `about:debugging#/runtime/this-firefox`.
2. Klicke auf **Temporäres Add-on laden...** und wähle die `manifest.json` aus diesem Ordner aus.


### 2. Einrichtung
1. Klick auf das Erweiterungs-Icon -> **Einstellungen**.
2. Gib einen Namen für dein Modul/Fach ein (z.B. "Programmierung").
4. **WICHTIG:** Gehe in ILIAS auf genau die Unterseite, auf der die restliche Zeit (*"Nächste Abgabefrist:"*) zu sehen ist. Kopiere **diesen** Link und füg ihn ein.
5. Klicke auf Speichern.



### 3. Datenschutz & Info
* **Passwort-Sicher:** Das Tool speichert **keine Passwörter**, sondern nutzt einfach deinen aktuellen, aktiven ILIAS-Login-Cookie im Browser.
* **100% Lokal:** Deine Links bleiben nur auf deinem PC. Kein Tracking, keine Cloud.
* **Tipp:** Wenn du bei ILIAS ausgeloggt bist bzw. der Login-Cookie abgelaufen ist, zeigt das Tool einen Fehler an. Einfach kurz bei ILIAS anmelden, dann geht wieder alles!

**Code ist anpassbar/erweiterbar für andere Webseiten mit Zeitangaben, welche du im Blick haben möchtest.**

Viel Erfolg bei den Abgaben :)
